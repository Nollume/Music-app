<template>
  <div class="border border-gray-200 p-3 mb-4 rounded">
    ...
    <div v-show="showForm">
      ...
      >
        <div class="mb-3">
          <label class="inline-block mb-2">{{ $t("edit.song_title") }}</label>
          <vee-field
            name="modified_name"
            type="text"
        ...
        <div class="mb-3">
          ...
          <vee-field
            ...
            type="text"
            class="block w-full py-1.5 px-3 text-gray-800 border border-gray-300 transition duration-500 focus:outline-none focus:border-black rounded"
            :placeholder="$t('edit.enter_genre')"
            @input="updateUnsavedFlag(true)"
          />
...
export default {
  ...
  methods: {
    ...
    async deleteSong() {
      const songStorageRef = reference(
        storage,
        `songs/${this.song.original_name}`
      );
      await deleteObject(songStorageRef);

      const songFirestoreRef = doc(songsCollection, this.song.docID);
      await deleteDoc(songFirestoreRef);
